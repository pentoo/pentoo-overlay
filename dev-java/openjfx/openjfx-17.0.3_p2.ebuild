...
#https://wiki.openjdk.org/display/OpenJFX/Building+OpenJFX#BuildingOpenJFX-LinuxDesktopBuildLinux

SRC_URI="https://github.com/openjdk/jfx17u/archive/refs/tags/${PV/_p/+}.tar.gz -> ${P}.tar.gz
...
src_prepare() {
	...
	sed -i "s#implementation name: SWT_FILE_NAME#implementation files(\"$(java-pkg_getjars swt-4.10)\")#" "${S}"/build.gradle || die
#	sed -i "s#implementation name: SWT_FILE_NAME#compile files(\"$(java-pkg_getjars swt-4.10)\")#" "${S}"/build.gradle || die

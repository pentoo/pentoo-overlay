...
# Autogenerated by ego_vendor_generator.sh
EGO_VENDOR=(
	...
	"github.com/tomoyamachi/reg v0.16.1"
...
src_unpack() {
	...
	for x in ${my_ego_vendor[@]}; do
		einfo "Vendoring ${my_ego_pn}/vendor/${x}"
		mkdir -p "vendor/${x}/" \
...
src_compile() {
	GOPATH="${WORKDIR}/${P}:$(get_golibdir_gopath)" \
		GOCACHE="${T}/go-cache" \
		go build -v -work -x -ldflags="-s -w -X config.Version=${PV}" ./... "${EGO_PN}" || die
}

src_install() {
	GOPATH="${WORKDIR}/${P}:$(get_golibdir_gopath)" \
		GOCACHE="${T}/go-cache" \
		go install -v -work -x -ldflags="-s -w -X config.Version=${PV}" ./... "${EGO_PN}" || die

...
QJS_COMMIT=3087a2ce5bcb66cc1fcd9f34d3e5ce3bd43a67d9
...
if [[ ${PV} == *9999 ]]; then
	...
	SRC_URI="mirror+https://github.com/radareorg/radare2/archive/${PV}.tar.gz -> ${P}.tar.gz
		mirror+https://github.com/quickjs-ng/quickjs/archive/${QJS_COMMIT}.tar.gz -> ${P}-qjs-${QJS_COMMIT}.tar.gz
		mirror+https://github.com/radareorg/sdb/archive/refs/tags/${SDB_VERSION}.tar.gz -> ${P}-sdb-${SDB_VERSION}.tar.gz
...
PATCHES=(
	...
	# https://github.com/radareorg/radare2/pull/26410
	"${FILESDIR}/${P}-io-shm-ldflags.patch"
)

src_prepare() {
	mv "${WORKDIR}/quickjs-${QJS_COMMIT}" subprojects/qjs || die
	mv "${WORKDIR}/sdb-${SDB_VERSION}" subprojects/sdb || die

	# https://github.com/radareorg/sdb/pull/324
	eapply "${FILESDIR}/${P}-sdb-link-cflags.patch"
	default

...
set_java() {
  ...
  if [ "${java_system/8/}" != "${java_system}" ]; then
    return 0
  fi
  wanted_java=$(eselect java-vm list | grep --color=never 8 | tr -d "[]" | awk '{print $2,$1}' | sort | head -n 1 | awk '{print $2}')
  if [ -n "${wanted_java}" ]; then
    ...
    printf "Failed to detect available jdk-8\n"
    return 0
  fi
}
